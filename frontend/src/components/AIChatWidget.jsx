...
import { X, Send, User, Cpu, BotMessageSquare, Network } from 'lucide-react';

export default function AIChatWidget() {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState([
    { sender: 'ai', text: 'Sisenco Core initialized. Neural link established with team databases. I have full context of all active projects, blockers, and team velocity. What insights do you require?' }
  ]);
      ...
      <div className={`fixed bottom-8 right-8 z-[100] flex w-[380px] sm:w-[400px] flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/80 shadow-[0_24px_80px_rgba(2,6,23,0.6)] backdrop-blur-2xl transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] origin-bottom-right ${isOpen ? 'scale-100 opacity-100 h-[600px]' : 'scale-0 opacity-0 h-0 pointer-events-none'}`}>
        ...
        <div className="flex items-center justify-between border-b border-white/10 bg-white/5 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold tracking-wide text-white">Sisenco Core</h3>
              <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-cyan-400 flex items-center gap-1.5 mt-0.5">
                <span className="h-1.5 w-1.5 rounded-full bg-cyan-400 animate-pulse"></span> Linked
              </p>
            </div>
          </div>
            ...
            <div key={idx} className={`flex w-full ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex max-w-[85%] gap-3 ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-white/10 ${msg.sender === 'user' ? 'bg-slate-900 text-slate-300' : 'bg-gradient-to-br from-[#5b7cfa]/20 to-cyan-400/20 text-cyan-400'}`}>
                  {msg.sender === 'user' ? <User size={14} /> : <Network size={14} />}
                </div>
                <div className={`rounded-[20px] p-4 text-sm leading-relaxed whitespace-pre-wrap ${msg.sender === 'user' ? 'rounded-tr-sm bg-gradient-to-r from-[#5b7cfa] to-[#2dd4bf] text-white shadow-lg' : 'rounded-tl-sm bg-white/5 text-slate-200 border border-white/10 shadow-sm'}`}>
                  {msg.text}
                </div>
              </div>
            </div>
